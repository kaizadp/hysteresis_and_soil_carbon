...
#  xlim(0,2.5)+
  ...
  #facet_grid(.~texture)+
  # 5%
  geom_point(data = peak2 %>% 
               filter(sat_level==5) %>% 
               dplyr::select(ppm, Drying, texture) %>% na.omit(), 
             aes(x = ppm, y = 0.5), color = "red")+
  geom_point(data = peak2 %>% 
               filter(sat_level==5) %>% 
               dplyr::select(ppm, Wetting) %>% na.omit(), 
             aes(x = ppm, y = 0.6), color = "blue")+
  # 35%
  geom_point(data = peak2 %>% 
               filter(sat_level==35) %>% 
               dplyr::select(ppm, Drying) %>% na.omit(), 
             aes(x = ppm, y = 1.0), color = "red")+
  geom_point(data = peak2 %>% 
               filter(sat_level==35) %>% 
               dplyr::select(ppm, Wetting) %>% na.omit(), 
             aes(x = ppm, y = 1.1), color = "blue")+
  # 50%
  geom_point(data = peak2 %>% 
               filter(sat_level==50) %>% 
               dplyr::select(ppm, Drying) %>% na.omit(), 
             aes(x = ppm, y = 1.5), color = "red")+
  geom_point(data = peak2 %>% 
               filter(sat_level==50) %>% 
               dplyr::select(ppm, Wetting) %>% na.omit(), 
             aes(x = ppm, y = 1.6), color = "blue")+
  # 75%
  geom_point(data = peak2 %>% 
               filter(sat_level==75) %>% 
               dplyr::select(ppm, Drying) %>% na.omit(), 
             aes(x = ppm, y = 2.0), color = "red")+
  geom_point(data = peak2 %>% 
               filter(sat_level==75) %>% 
               dplyr::select(ppm, Wetting) %>% na.omit(), 
             aes(x = ppm, y = 2.1), color = "blue")+
  # 100%
  geom_point(data = peak2 %>% 
               filter(sat_level==100) %>% 
               dplyr::select(ppm, Drying) %>% na.omit(), 
             aes(x = ppm, y = 2.5), color = "red")+
  geom_point(data = peak2 %>% 
               filter(sat_level==100) %>% 
               dplyr::select(ppm, Wetting) %>% na.omit(), 
             aes(x = ppm, y = 2.6), color = "blue")+
  geom_rect(data = peak2, 
            aes(xmin = DMSO_start, xmax = WATER_stop, ymin = 0.2, ymax = 3.5),
            fill = "grey90", alpha = 0.015)+
  NULL
)
