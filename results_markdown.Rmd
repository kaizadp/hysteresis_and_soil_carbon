...
fticr_biplot = 
  ...
  scale_x_continuous(limits = c(-3,3.5), oob=rescale_none)+
  scale_y_continuous(limits = c(-3,3.5), oob=rescale_none)+
  labs(title = "SCL",
       caption = "core 25 is out of bounds, influenced by aliphatic, ~10 on Axis1")+
  theme_kp()+
  theme(panel.grid = element_blank(),
...
fticr_eucl = 
  ...
  labs(x = "% saturation",
       y = "drying-rewetting \n Euclidean distance")+
  geom_hline(yintercept = 1.73, linetype = "dashed")+
  
  geom_text(data = eucl_label %>% filter(type=="text"), aes(x = x, y = y), 
            label = "avg. within-group distance", angle=90, hjust="left", size=3)+
  geom_curve(data = eucl_label %>% filter(type=="curve"), aes(x = x, xend = xend, y = y, yend = yend),
             curvature = -0.2)+
  
    facet_grid(.~texture.x)+

  #annotate("text", label = "avg within-group distance", x = 0.7, y = 30, angle=90, size=3, hjust="left")+
  #annotate("curve", x = 0.7, xend = 0.7, y = 19, yend = 28, curvature=-0.2)+
  theme_kp()
